...
StreamWriter sw;
       ...
       /* void OnGUI()
        ...
        IEnumerator printfps(int maxTime)
        {
            ...
            do
            {
                if (false == File.Exists(fullpth  + "FPSLog_" + n.ToString() +"_"+ scene.name+ ".txt"))
                {
                    //var file = File.CreateText(fullpth  + "FPSLog_" + n.ToString() + ".txt");
                    sw = new StreamWriter(fullpth + "FPSLog_" + n.ToString()+"_" + scene.name + ".txt");
             //       file.Close();
                    break;
                }
                else n++;
            ...
            {
                //Debug.Log("time : " + time);
                Debug.Log("Time["+time+"]"+"Scene name:" + scene.name + " " + logfps.ToString("N0") + "fps (" + logmsec.ToString("N1") + " msec )");
                sw.WriteLine("Time[" + time + "]" + "Scene name:" + scene.name + " " + logfps.ToString("N0") + "fps (" + logmsec.ToString("N1") + " msec )");
