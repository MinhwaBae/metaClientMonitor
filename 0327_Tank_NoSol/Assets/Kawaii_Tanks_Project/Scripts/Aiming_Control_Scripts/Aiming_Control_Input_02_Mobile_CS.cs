...
namespace ChobiAssets.KTP
{
    ...
    public class Aiming_Control_Input_02_Mobile_CS : Aiming_Control_Input_00_Base_CS
    {
        ...
        public override void Prepare(Aiming_Control_CS aimingScript)
        {
            ...
            // Enable the "useAutoLead".
            aimingScript.useAutoLead = true;
        }
        ...
        void Auto_Lock_On()
        {
            // Check the auto lock button is down.
            if (isAutoLockButtonDown == false && Key_Bindings_CS.IsAutoLockButtonPressing)
            {
                isAutoLockButtonDown = true;
                return;
            }

            // Check the lock button is up.
            if (isAutoLockButtonDown && Key_Bindings_CS.IsAutoLockButtonPressing == false)
            {
                var deltaX = Key_Bindings_CS.AutoLockButtonDownPosition.x - Key_Bindings_CS.AutoLockButtonUpPosition.x;
                if (deltaX > 0.0f)
                { // Left.
